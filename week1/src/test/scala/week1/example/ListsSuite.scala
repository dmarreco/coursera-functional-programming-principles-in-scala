 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum with empty list is zero") {
    assert(sum(List()) === 0)
  }

  test("sum of a single number is this number") {
    assert(sum(List(123)) === 123)
  }
  ...
  test("max of negatives and zero is zero") {
    assert(max(List(-100, -200, 0)) === 0)
  }

  test("max of a single value is itself") {
    assert(max(List(12345)) === 12345)
  }
